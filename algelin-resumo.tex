...
Vamos tomar $\beta = \{ a_1, \ldots, a_n \}$ e $\gamma = \{ b_1, \ldots, b_n \}$ duas bases ordenadas de um espaço n-dimensional $V$. Note que podemos escrever cada vetor da base $\gamma$ como combinação linear dos vetores de $\beta$ da seguinte forma:
...
onde cada $\alpha_{ij}$ é um escalar no corpo $\mathbb{F}$. Assim, para cada $i \in \{1, 2, \ldots, n \}$, o vetor das coordenadas de $b_i$ na base $\gamma$ é dado por
$$
[c_i]_\beta =
\begin{bmatrix} 
\alpha_{1i} \\
...
Note que cada coluna é formada pelas coordenadas de $b_1, \ldots, b_n$ em relação à base $\beta$.

\subsection{Exercícios Resolvidos}

Como verificar se $S_A$ é subespaço vetorial de $S$?

Basta verificar se $c \alpha + d \beta \in S_A$, onde $c,d \in \mathbb{F}$ e $\alpha, \beta \in \mathbb{V}$​​.
...
1. $IU = UI = U$;
2. $U(T_1 +T_2) = UT_1 + UT_2$ e $(T_1 + T_2)U = T_1U + T_2U$;
3. $\alpha(UT_1)) = (\alpha U) T_1 = U(\alpha T_1)$, onde $\alpha \in \mathbb{F}$.
...
1. $T$ é invertível.
2. $T$ é não singular (ou seja, é injetora).
3. $T$ é sobrejetora.
...
Vamos começar definindo $V$ um espaço vetorial de dimensão $n$ e $W$ um espaço vetorial de dimensão $m$, ambos sobre o corpo $\mathbb{F}$. Definimos também $\beta = \{ v_1, \ldots, v_n\}$ uma base ordenada para $V$ e $\gamma = \{ w_1, \ldots, w_m\}$ uma base ordenada de $W$.

Observe que se $T$ é uma transformação linear de $V$ em $W$, então $T$ é definido de maneira única pela sua ação nos vetores $v_j$ de $\beta$. Isto é, cada um dos $n$ vetores $T(v_j)$ pode ser escrito de maneira única como uma combinação linear
...
A partir disso, podemos definir a matriz $A$, $m \times n$ definida por $A(i,j) = c_{ij}$ e dizemos que $A$ é a \textbf{matriz de $T$ relativa às bases $\beta$ e $\gamma$}. Por simplicidade, denotaremos $A = [T]_{\beta,\gamma}$.
...
% Axler p. 85
% Dar exemplo da matriz de diferenciação

\subsection{Exercícios Resolvidos}

1. Encontre a \textbf{inversa} de $T(x_1, x_2) = (x_1 + x_2, x_1)$, onde $T$ é um operador linear em $\mathbb{F}^2$.
